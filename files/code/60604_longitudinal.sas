/*
   Données provenant du livre
   Applied Longitudinal Analysis (2011, 2nd Ed) 
   G. Fitzmaurice, N. Laird, and J. Ware.
   https://content.sph.harvard.edu/fitzmaur/ala2e/
   
   Le code est extrait d'un article de blogue de Rick Wicklin
   https://blogs.sas.com/content/iml/2019/12/03/longitudinal-data-response-profile-model.html
*/

/* Lire les données dans SAS, et créer un jeu de données (TLC) en format long */

data tlc;
   input id traitement $ plomb0 plomb1 plomb4 plomb6;
   y=plomb0; temps=0; output;
   y=plomb1; temps=1; output;
   y=plomb4; temps=4; output;
   y=plomb6; temps=6; output;
   drop plomb0 plomb1 plomb4 plomb6;
   label y = "Niveau de plomb dans le sang (mcg/dL)";
datalines;
...
P 1
;
/* Pour infos sur l'énoncé LEGENDITEM, voir
   https://blogs.sas.com/content/iml/2018/02/12/merged-legends.html 
*/

title "Profil de réponse d'enfants exposés au plomb";
/* Graphique spaghetti */
proc sgplot data=tlc dattrmap=Ordre;
   series x=temps y=Y / group=ID groupLC=traitement break lineattrs=(pattern=solid)
                       attrid=Treat;
...
/* Créer une copie de "temps" pour l'ajouter comme variable dans l'énoncé REPEATED. */
data TLCr;
set tlc;
t = temps;       /* copie catégorielle de temps */
run;



/* Analyse des profils avec une structure AR(1) pour les erreurs*/
proc mixed data=TLCr;
   class id temps(ref='0') traitement(ref='P');
   model y = traitement temps traitement*temps / solution outpred=MixedOut;
   repeated temps / type=sp(pow)(temps) subject=id r rcorr=1;   /* mesures répétées */
run;
